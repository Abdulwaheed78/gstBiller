...
class AdminController extends Controller
{
    public function dashboard()
    {
        ...
        $log = Logs::with('user')->orderBy('id', 'desc')->limit(10)->get();

        $user = User::orderBy('id', 'desc')->limit(10)->get();

        return view('admin.dashboard', compact('data', 'log', 'user'));
    }

    public function index()
    {
        $data = User::where('is_deleted', 'no')->orderBy('id', 'desc')->get();
    ...
    public function create(Request $request)
    {
        $request->validate([
            ...
            'image' => 'nullable|image|mimes:jpeg,png,jpg,gif,svg', // Optional: Add validation for image
            ...
            'user_type' => 'required|in:admin,vendor' // Validate user_type values
        ]);
        ...
        try {
            ...
            if ($user->save()) {
                //Adding Log For Creaton
                $users = Auth::user();
                $log = new Logs();
                $log->table_name = 'user';
                $log->type_name = $request->user_type . ' Created';
            ...
            return redirect()->route('admin.add')->with('error', 'Error Occurred While Creating User');
        } catch (\Exception $e) {
            return redirect()->route('admin.add')->with('error', 'An unexpected error occurred: ' . $e->getMessage());
        }
    }
    ...
    public function update(Request $request)
    {
        ...
        if (!empty($request->password) && $request->password !== $request->password_confirmation) {
            return redirect()->route('admin.edit', ['id' => $user->id])->with('error', 'Password and Confirm Passwords are not Matching');
        }
        $user->name = $request->name;
        $user->email = $request->email;
        $user->phone = $request->phone;
        ...
        if ($user->save()) {
            ...
            return redirect()->route('admin.edit', ['id' => $user->id])->with('error', 'Error Occurred While Updating User');
        }
    }

    public function destroy_user(Request $request)
    {
        $user = User::where('id', $request->id)->update(['is_deleted' => 'yes']);
        if ($user > 0) {
            $usertype = User::find($request->id);
            $users = Auth::user();
            //first entry on log that user logged in
            $log = new Logs();
            $log->table_name = 'user';
            $log->type_name = $usertype->user_type . 'Delete';
            $log->action_name = 'delete';
            $log->item_id = $request->id;
            $log->uid = $users->id;
            $log->save();
            return redirect()->route('admin.index')->with('success', 'User Deleted Successfully');
        } else {
            return redirect()->route('admin.index')->with('error', 'Error While Deleting User');
        }
    }
    ...
    public function update_profile(Request $request)
    {
        ...
        $user = User::find($request->id);
        if (!empty($request->password) && $request->password !== $request->password_confirmation) {
            return redirect()->route('admin.profile', ['id' => $user->id])->with('error', 'Password and Confirm Passwords are not Matching');
        }
        $user->name = $request->name;
        $user->email = $request->email;
        $user->phone = $request->phone;
        ...
        if ($user->save()) {
            ...
            return redirect()->route('admin.profile', ['id' => $user->id])->with('error', 'Error Occurred While Updating Profile');
        }
    }
    ...
    {
        $data = Bills::where('is_deleted', 'no')->with('creator')->orderBy('id', 'desc')->get();
        return view('admin.bills.index', compact('data'));
    }
    ...
    public function bill_create(Request $request)
    {
        ...
        if ($bill->save() > 0) {
            ...
            return view('admin.bills.update', compact('data'))->with('success', 'Bill Created Successfully Now Add Products');
        }

        return redirect()->route('admin.bills.add')->with('error', 'Error Happens While Creating Bill Created Successfully');
    }

    public function bill_edit($id)
    {
        $data = Bills::with('trans')->find($id);
        return view('admin.bills.update', compact('data'));
    }
    ...
    public function bill_update(Request $request)
    {
        ...
        $data = $bill;
        return view('admin.bills.update', compact('data'))->with('error', 'Error Happens While Updating Bill');
    }
    ...
    public function bill_destroy($id)
    {
        ...
        return redirect()->route('admin.bills.index')->with('error', 'Bill Not Deleted');
    }
    ...
    public function bill_add_product($id)
    {
        ...
        return view('admin.bills.add_product', compact('data', 'id', 'bill'));
    }
    ...
    public function bill_create_product(Request $request)
    {
        ...
        return redirect()->route('admin.bills.edit', ['id' => $billId])->with('success', 'Products added successfully!');
    }
    ...
    public function bill_edit_product($id)
    {
        ...
        return view('admin.bills.edit', compact('data', 'bill'));
    }
    ...
    public function bill_update_product(Request $request, $id)
    {
        ...
        if (!$trans) {
            return redirect()->route('admin.bills.edit', ['id' => $billId])->with('error', 'Transaction not found!');
        }
        ...
        $bill = Bills::find($billId);
        if (!$bill) {
            return redirect()->route('admin.bills.edit', ['id' => $billId])->with('error', 'Bill not found!');
        }
        ...
        // Redirect or return a response after saving
        return redirect()->route('admin.bills.edit', ['id' => $billId])->with('success', 'Product updated successfully!');
    }
    ...
    public function bills_destroy_product($id)
    {
        ...
        if ($data->delete()) {
            ...
            $log->uid = $users->id;
            $log->save();
            return redirect()->route('admin.bills.edit', ['id' => $data->bill->id])->with('success', 'Record Deleted Successfully');
        }
        return redirect()->route('admin.bills.edit', ['id' => $data->bill->id])->with('error', 'Error While Deleting Record');
    }
    ...
    public function generateInvoice($id)
    {
        ...
        $pdf = PDF::loadView('admin.bills.download', compact('data'));
    ...
    {
        $data = Logs::with('user')->orderBy('id', 'desc')->get();
